...
class ProfileTest(unittest.TestCase):
    ...
    def test_insert_profile(self):
        ...
        self.pl.insert_profile(profile1["Options"], profile1["Welcome"], profile1["Login"], profile1["Authentication"])
        self.assertEqual(count+1, self.dbcon.mycol.estimated_document_count())

    def test_get_profile(self):
        self.dbcon.add_profile(profile1)
        profile_test_1 = self.pl.get_profile(profile1["Options"], profile1["Welcome"], profile1["Login"],
                                      profile1["Authentication"])
        self.assertEqual(profile_test_1, profile1["_id"])

        profile_test_2 = self.pl.get_profile(profile1["Options"], profile1["Welcome"] + "welcome", profile1["Login"],
                                      profile1["Authentication"])
