    ...
    def parse_buffer(buffer, ip):
        ...
        while i < len(buffer):
            ...
            if byte_iac == 255:  # Found an IAC!
                ...
                if byte_command in ByteParser.telnet_commands:  # Found a known command!
                    if 250 < byte_command:  # Found a telnet option command!
                        ...
                        if byte_option in ByteParser.telnet_command_options:  # Found a known option!
                            logging.info(f"FROM {ip} IAC {ByteParser.telnet_commands[byte_command]} : {ByteParser.telnet_command_options[byte_option]}")
